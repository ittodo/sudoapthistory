...
(()=>{
  ...
  const storedPrice=v=>NodoHomeSearchModel.filterPrice(type,v,true),inputPrice=v=>NodoHomeSearchModel.filterPrice(type,v);
  ...
  function remember(){if(!ready||restoring)return;states[type]={pL:String(storedPrice(gv('pL'))),pH:String(storedPrice(gv('pH'))),sc,sa};try{localStorage.setItem('nodo:home-prices:v1',JSON.stringify(states));}catch{}const u=new URL(location.href);u.searchParams.set('tenure',type);u.searchParams.set('homePrices',JSON.stringify(states));history.replaceState(history.state,'',u);}
  function useState(){const s=states[type]||{pL:'',pH:'',sc:'ld',sa:false};document.getElementById('pL').value=inputPrice(s.pL);document.getElementById('pH').value=inputPrice(s.pH);sc=NodoHomeSearchModel.sortKey(type,s.sc);sa=s.sa;}
  function sync(){
    ...
    if(rental()){document.getElementById('priceFilterLabel').textContent=type==='jeonse'?'전세 보증금':'월 환산액';document.getElementById('priceFilterUnit').textContent=type==='jeonse'?'억':'만원';document.getElementById('priceColumnLabel').textContent=type==='jeonse'?'최신 전세 보증금':'최신 월세';}
  ...
  function price(x){if(!rental())return undefined;const t=record(x);return t?(type==='jeonse'?inputPrice(t.deposit):t.value):null;}
  function priceText(x){if(!rental())return x.lp==null?'거래 없음':null;const t=record(x);if(!t)return '거래 없음';return type==='jeonse'?fmtPrice(inputPrice(t.deposit)):`${t.value==null?"":"월 환산 "+money(t.value)+"<br>"}<small>${t.equivalent==null?"":"보증금 환산 "+fmtPrice(t.equivalent/10000)+"<br>"}보증금 ${money(t.deposit)} / 월세 ${money(t.rent)}</small>`;}
  ...
  window.NodoHomeSearchView={storedPrice,inputPrice,loadIndex,install,rental,price,priceText,sortValue,sortKey:key=>NodoHomeSearchModel.sortKey(type,key),selection,decorate,remember,initialState:useState,date:x=>rental()?(record(x)?.date||'—'):null};
})();
