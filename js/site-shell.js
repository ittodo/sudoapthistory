(() => {
  ...
  const section = path.startsWith('/div') ? 'company' : /\/(calc|savings)/.test(path) ? 'calc' : path.startsWith('/board') ? 'board' : 'housing';
  const groups = {housing:[['단지 검색','/'],['지도','/map/'],['지역·단지 비교','/compare/'],['시장 동향','/market/'],['가격대 통계','/stats/'],['실거래가','/trades/'],['전월세','/trades/contracts.html'],['단지정보','/kapt-v2.html'],['정책','/policy.html']],company:[['배당·소각','/div/'],['기업 목록','/div/stocks/']],calc:[['부동산 계산기','/calc/'],['예적금 계산기','/calc/savings.html']],board:[]};
  ...
  header.innerHTML=`<a class="nodo-skip" href="#nodo-content">본문으로 건너뛰기</a><div class="nodo-top"><a href="/" class="nodo-brand" aria-label="노도스트림 홈">nodo<span>stream</span></a><nav class="nodo-primary" aria-label="주 메뉴">${[['부동산','/','housing'],['기업','/div/','company'],['계산기','/calc/','calc'],['게시판','/board/','board']].map(([n,h,s])=>anchor(n,h,s===section)).join('')}</nav><div class="nodo-tools"><label><span class="sr-only" style="position:absolute;clip:rect(0,0,0,0)">화면 테마</span><select id="nodo-theme"><option value="system">시스템 설정</option><option value="light">라이트</option><option value="dark">다크</option></select></label><a href="/account/">계정</a></div></div>${groups[section].length?`<nav class="nodo-subnav" aria-label="${section==='housing'?'부동산':'영역'} 메뉴">${groups[section].map(([n,h])=>anchor(n,h,same(h))).join('')}</nav>`:''}`;
