(async function() {
  ...
  function render(){
    const s=M.summary(selected);$('dailyStats').innerHTML=[['유효 거래',s.count,'',`직거래 ${s.direct.toLocaleString()}건 포함`],['역대 신고가',s.high,'up','각 계약일 이전 역대 최고가 초과'],['직전 대비 상승',s.up,'up','직전 계약일 최저가격 초과'],['직전 대비 하락',s.down,'down','직전 계약일 최저가격 미만'],['역대 신저가',s.low,'down','각 계약일 이전 역대 최저가 미만'],['해제 거래',s.cancelled,'','선택 기간에 계약된 거래 중 해제'],['원천에서 사라짐',s.missing,'','해제 여부 미확인 · 유효 거래 제외']].map(([title,n,cls,note])=>`<article class="daily-stat ${cls}"><p>${title}</p><strong>${n.toLocaleString()}<small> 건</small></strong><small>${note}</small></article>`).join('');
