(async function() {
  ...
  function restore(){
    ...
    kind=['all','high','low','down','inactive'].includes(p.get('kind'))?p.get('kind'):'all';
    sort=['price','drop','area'].includes(p.get('sort'))?p.get('sort'):kind==='down'?'drop':'price';
  ...
  function render(){
    const s=M.summary(selected);$('dailyStats').innerHTML=[['유효 거래',s.count,'',`직거래 ${s.direct.toLocaleString()}건 포함`],['역대 신고가',s.high,'up','각 계약일 이전 역대 최고가 초과'],['직전 대비 하락',s.down,'down','직전 계약일 최저가격 미만'],['역대 신저가',s.low,'down','각 계약일 이전 역대 최저가 미만'],['해제 거래',s.cancelled,'','선택 기간에 계약된 거래 중 해제'],['원천에서 사라짐',s.missing,'','해제 여부 미확인 · 유효 거래 제외']].map(([title,n,cls,note])=>`<article class="daily-stat ${cls}"><p>${title}</p><strong>${n.toLocaleString()}<small> 건</small></strong><small>${note}</small></article>`).join('');
    ...
    const drop=t=>t.previousMin?(t.p-t.previousMin)/t.previousMin:Infinity;
    list.sort((a,b)=>(sort==='drop'?(drop(a)-drop(b)):sort==='area'?(a.a-b.a):b.p-a.p)||a.c.n.localeCompare(b.c.n)||a.id.localeCompare(b.id));
    ...
    $('dailyRows').innerHTML=list.length?list.slice(0,limit).map(t=>{
      const diff=t.previousMin==null?null:t.p-t.previousMin,pct=diff==null?null:diff/t.previousMin*100;
  ...
  function chart(){
    ...
    $('dailyChart').innerHTML=days.map(s=>`<button data-date="${s.d}" ${s.d===day?'aria-current="date"':''} ${s.d<client.index.minDate||s.d>client.index.maxDate?'disabled':''} aria-label="${s.d} ${s.d<client.index.minDate||s.d>client.index.maxDate?'보유 자료 없음':`전체 ${s.count}건 신고가 ${s.high}건 하락 ${s.down}건`}" title="${s.d} · ${s.d<client.index.minDate||s.d>client.index.maxDate?'보유 자료 없음':`전체 ${s.count} · 신고가 ${s.high} · 하락 ${s.down}`}"><span class="daily-bar-stack"><i style="height:${s.count/max*100}%"></i><i style="height:${s.high/max*100}%"></i><i style="height:${s.down/max*100}%"></i></span><small>${s.d.slice(5).replace('-','.')}</small></button>`).join('');
  }
  async function load(){
  ...
  document.querySelector('.daily-results .daily-tabs').onclick=e=>{const button=e.target.closest('[data-kind]');if(!button||!client)return;kind=button.dataset.kind;sort=kind==='down'?'drop':'price';$('dailySort').value=sort;limit=50;render();};
