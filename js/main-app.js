// nodostream main application script - extracted from index.html
...
async function renderDetail(x){
  ...
  // 아파트별 댓글 연동
  const apid='apt_'+x.i;
  if(window._aptCommentsReady){changePageId(apid);}
  else{initComments(apid,'#apt-comments');window._aptCommentsReady=true;}
}
...
function showOnMap(x){
  ...
  if(syncCoord){
    ...
    setTimeout(()=>{
      if(token!==mapSelectionToken||curParcelOverlay) return;
      LMAP.invalidateSize();
