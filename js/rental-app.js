...
(async()=>{
 ...
 function request(action,extras={}){if(!worker){worker=new Worker('/js/rental-worker.js?v=20260921-shared-cache6');worker.onmessage=({data})=>{const p=pending.get(data.id);if(!p)return;pending.delete(data.id);data.error?p.reject(Error(data.error)):p.resolve(data);};worker.onerror=()=>{for(const p of pending.values())p.reject(Error('전월세 계산을 시작하지 못했습니다. 새로고침해 주세요.'));pending.clear();};}const id=++seq;return new Promise((resolve,reject)=>{pending.set(id,{resolve,reject});worker.postMessage({id,action,...extras});});}
 ...
 function warmNextMonth(){
  ...
  request('prefetch',{settings:{map:true,type:settings.type,day:settings.day,region:settings.region,endMonth}}).then(result=>{
 ...
 async function refresh(){const id=++frame;try{
  ...
  if(settings.map&&map){const b=map.getBounds().pad(.12);settings.bounds={south:b.getSouth(),north:b.getNorth(),west:b.getWest(),east:b.getEast()};}
