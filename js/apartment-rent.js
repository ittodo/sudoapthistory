...
(()=>{
 ...
 let loading,reader,charts=[],historyIndex,historyWorker,sequence=0;
 ...
 async function render({apartment,area,panel,current,script}){
  clear();const [m,history]=await Promise.all([index(),historyManifest()]);if(!current())return;
  const ref=m.records[apartment.id]||{areas:[],districts:[...new Set([apartment.id,...apartment.sources].map(x=>String(x).match(/^(\d{5})-/)?.[1]).filter(Boolean))]};
  const data=ref.shard?await reader('data/apartment-rent/'+ref.shard+'.json'):null;if(!current())return;const rows=data?.[apartment.id]?.rows||[];
  const p=new URLSearchParams(location.search),years=[...new Set([...history.historyYears,...Object.values(m.coverage).flatMap(x=>Object.keys(x)).map(x=>x.slice(0,4))])].sort().reverse(),historical=new Map();
  panel.innerHTML=`<div class="nodo-card"><h2>전월세 계약 내역</h2><div class="rent-filters"><label class="nodo-field">거래 유형<select id="rent-type"><option value="all">전체</option><option value="jeonse">전세</option><option value="monthly">월세</option></select></label><label class="nodo-field">기간<select id="rent-period"><option value="recent">최근 12개월</option>${years.map(y=>`<option value="${y}">${y}년</option>`).join('')}</select></label><label class="nodo-field">전용면적<select id="rent-area"><option value="all">전체 면적</option>${ref.areas.map(a=>`<option value="${a}">${a}㎡</option>`).join('')}</select></label></div><p id="rent-status" class="nodo-muted" role="status"></p><div class="rent-workspace"><div class="rent-chart-column"><div class="rent-charts"><div><h3>전세·월세 보증금</h3><div class="nodo-chart"><canvas id="rent-deposit-chart"></canvas></div></div><div><h3>월세</h3><div class="nodo-chart"><canvas id="rent-monthly-chart"></canvas></div></div></div><p class="nodo-muted">같은 거래 유형·선택 면적의 월평균입니다. 해제 거래는 통계에서 제외하며, 거래가 없는 달은 연결하지 않습니다.</p></div><div class="rent-table-column"><div class="nodo-table-wrap"><table class="nodo-table"><thead><tr><th>계약일</th><th>전용면적 · 층</th><th>유형</th><th>보증금</th><th>월세</th><th>계약 구분 · 기간</th><th>종전 보증금 · 월세</th><th>갱신권</th></tr></thead><tbody id="rent-rows"></tbody></table></div><button id="rent-more" class="nodo-button">더 보기</button></div></div></div>`;
  ...
  const update=async()=>{const rev=++revision,periodKey=period.value;clear();const requested=months(periodKey);
   ...
   if(oldYear){
    ...
    try{if(!historical.has(periodKey))historical.set(periodKey,Promise.all([...new Set(requested.map(m=>m.slice(0,4)))].map(year=>historyRows(apartment,year))).then(parts=>parts.flat().sort((a,b)=>b.date.localeCompare(a.date))));source=await historical.get(periodKey);}catch(e){historical.delete(periodKey);if(current()&&rev===revision){$('rent-status').textContent=e.message;$('rent-rows').innerHTML='';$('rent-more').hidden=true;}return;}
    if(!current()||rev!==revision)return;
    for(const r of source)if(![...size.options].some(o=>o.value===String(r.area)))size.add(new Option(r.area+'㎡',String(r.area)));
    if(p.get('rentArea')&&[...size.options].some(o=>o.value===p.get('rentArea'))&&!size.dataset.changed)size.value=p.get('rentArea');
   }
   const selected=source.filter(r=>requested.includes(r.date.slice(0,7).replace('-',''))&&(type.value==='all'||r.category===type.value)&&(size.value==='all'||(size.value.startsWith('group:')?Math.round(r.area)===Number(size.value.slice(6)):String(r.area)===size.value)));
   const missing=requested.filter(x=>ref.districts.some(d=>oldYear?!history.coverage[d]?.[x.slice(0,4)+'-'+x.slice(4)]:!m.coverage[d]?.[x]));const stamp=ref.districts.flatMap(d=>Object.values(m.coverage[d]||{})).sort().at(-1);
   $('rent-status').textContent=`${selected.length.toLocaleString()}건${oldYear?' · 과거 계약 이력':' · 최근 확인 '+(stamp?.slice(0,10)||'미확인')}${missing.length?' · 미수집 기간 '+missing.join(', '):''}`;
  ...
  for(const select of [type,period,size])select.onchange=()=>{if(select===size)size.dataset.changed='1';limit=50;const u=new URL(location.href);u.searchParams.set('rentType',type.value);u.searchParams.set('rentPeriod',period.value);u.searchParams.set('rentArea',size.value);history[window.NodoApartmentLinks?.embedded?'replaceState':'pushState'](null,'',u);window.NodoApartmentLinks?.sync();update();};
