...
(()=>{
 ...
 async function render({apartment,area,panel,current,script}){
  ...
  const ref=m.records[apartment.id]||{areas:apartment.rentalAreas||[],districts:[...new Set([apartment.id,...apartment.sources].map(x=>String(x).match(/^(\d{5})-/)?.[1]).filter(Boolean))]};
  ...
  const p=new URLSearchParams(location.search),years=[...new Set([...archive.historyYears,...Object.values(m.coverage).flatMap(x=>Object.keys(x)).map(x=>x.slice(0,4))])].sort().reverse(),historical=new Map();
  ...
  const update=async()=>{const rev=++revision,periodKey=period.value;clear();const requested=months(periodKey);
   ...
   const missing=requested.filter(x=>ref.districts.some(d=>oldYear?!archive.coverage[d]?.[x.slice(0,4)+'-'+x.slice(4)]:!m.coverage[d]?.[x]));const stamp=ref.districts.flatMap(d=>Object.values(m.coverage[d]||{})).sort().at(-1);
