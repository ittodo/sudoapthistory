...
const migration='0004_member_library.sql';
const approvedHash='c7711ed3171449723e2d05ba496752500b2c2e7d87309586431acea94f03086c';
const normalize=s=>s.trim().replace(/;$/,'').replace(/\s+/g,' ').toLowerCase();
export async function ensureLibrarySchema(env=process.env,request=fetch,source=readFileSync(new URL('../cloudflare/migrations/'+migration,import.meta.url),'utf8')){
 ...
 assert.equal(createHash('sha256').update(source.replaceAll('\r\n','\n')).digest('hex'),approvedHash,'Only reviewed additive library SQL is allowed');
 const query=async(sql,params=[])=>{const response=await request(`https://api.cloudflare.com/client/v4/accounts/${env.CLOUDFLARE_ACCOUNT_ID}/d1/database/${env.CLOUDFLARE_PRODUCTION_D1_ID}/query`,{method:'POST',headers:{Authorization:`Bearer ${env.CLOUDFLARE_API_TOKEN}`,'Content-Type':'application/json'},body:JSON.stringify({sql,params}),redirect:'error',signal:AbortSignal.timeout(30000)});if(!response.ok)throw Error('Library schema request failed: '+response.status);const data=await response.json();if(data.success!==true||!data.result?.length||data.result.some(r=>r.success!==true))throw Error('Library schema request rejected');return data.result[0].results;};
 const applied=(await query('SELECT name FROM d1_migrations ORDER BY id')).map(x=>x.name);for(const name of ['0001_initial.sql','0002_withdrawal_requests.sql','0003_board_pins.sql'])assert.ok(applied.includes(name),'Earlier migration missing');
...
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href){try{console.log('Approved member library tables: '+await ensureLibrarySchema());}catch(e){console.error(e.message);process.exitCode=1;}}
