...
export async function auth(r:Request,env:Env){
 ...
 if(url.pathname==='/auth/google'){
  ...
  if(withdrawal&&!reauth)throw new HttpError(400,'WITHDRAWAL_INVALID','계정 확인이 필요합니다.');
 ...
 try {
  ...
  if(stored.reauth_user){
   ...
   if(stored.withdrawal_id){
    await finishWithdrawalAuth(r,env,stored.withdrawal_id,result.access_token,result.expires_in??0);
    return new Response(null,{status:302,headers:{Location:env.SITE_ORIGIN+'/account/?withdrawal=confirm','Cache-Control':'no-store','Set-Cookie':setCookie('__Host-nodo_oauth','',0)}});
   }
  ...
  const existing=await env.DB.prepare('SELECT id,status,recovery_deadline FROM users WHERE google_sub=?').bind(claims.sub).first<{id:string,status:string,recovery_deadline:number|null}>();
  if(existing?.status==='withdrawn'){
   if(existing.recovery_deadline!==null&&existing.recovery_deadline>timestamp)return issueRecovery(env,existing.id);
   await expireAccounts(env.DB,timestamp,existing.id);
  }
  if(withdrawalEnabled(env)){
   const identityKey=await identityHash(env,claims.sub);
   const blocked="NOT EXISTS(SELECT 1 FROM withdrawal_requests WHERE identity_hash=? AND status='processing' AND lock_until>?)";
   await env.DB.batch([
    env.DB.prepare(`INSERT INTO users(id,google_sub,email) SELECT ?,?,? WHERE ${blocked} ON CONFLICT(google_sub) DO UPDATE SET email=excluded.email WHERE users.status='active'`).bind(id,claims.sub,claims.email,identityKey,timestamp),
    env.DB.prepare(`INSERT INTO sessions(token_hash,user_id,expires_at,authenticated_at) SELECT ?,id,?,? FROM users WHERE google_sub=? AND status='active' AND ${blocked}`).bind(await hash(raw),timestamp+604800,timestamp,claims.sub,identityKey,timestamp)
   ]);
   if(!await env.DB.prepare('SELECT 1 FROM sessions WHERE token_hash=?').bind(await hash(raw)).first())throw new Error('withdrawal in progress');
  }else{
  await env.DB.batch([
   env.DB.prepare("INSERT INTO users(id,google_sub,email) VALUES(?,?,?) ON CONFLICT(google_sub) DO UPDATE SET email=excluded.email WHERE users.status='active'").bind(id,claims.sub,claims.email),
   env.DB.prepare("INSERT INTO sessions(token_hash,user_id,expires_at,authenticated_at) SELECT ?,id,?,? FROM users WHERE google_sub=? AND status='active'").bind(await hash(raw),timestamp+604800,timestamp,claims.sub)
  ]);
  }
  if(!await env.DB.prepare('SELECT 1 FROM sessions WHERE token_hash=?').bind(await hash(raw)).first())throw new Error('account unavailable');
