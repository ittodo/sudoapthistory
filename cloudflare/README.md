...
`wrangler.jsonc` uses compatibility date 2026-08-01, supported by the pinned local workerd. Top-level settings represent the newly created empty free staging D1 database; its ID is a public resource identifier, not a credential. Staging is deployed with test users; do not reset it. `--env production` selects the separate production database. External status is recorded in `../docs/cloudflare-implementation-status.md`. Production CPU cap requires the approved paid plan; do not enable a paid plan automatically. Static assets are routed first, except `/api/*` and `/auth/*`; no SPA fallback. `www` redirection belongs to the zone redirect configuration, not this asset-first Worker.

## Safe startup and authentication

Production remains `AUTH_ENABLED=false`, `MAINTENANCE=true`; staging is active. Configure Google credentials through Worker secret facilities; never put secrets in vars, Git or logs. Set staging `SITE_ORIGIN` to its actual HTTPS origin.
...
Account withdrawal requires a start receipt followed by `/auth/google?reauth=1&withdrawal=1&returnTo=/account/`, bound to the current session and same Google `sub`. It uses Google's `prompt=select_account`. Successful confirmation prepares a 10-minute withdrawal request with an encrypted access token; it does not itself withdraw the member. **This confirms the selected Google account; it does not promise that Google asked for a password again.** A different account or changed/expired session is rejected without replacing the original session. Real Google console callback and public-login checks remain required before production launch.
...
- `POST /api/logout`: `{ok:true}`. `DELETE /api/account` aliases withdrawal confirmation and returns separate account/Google results; it requires the withdrawal receipt and recent same-account verification.
...
- `GET /api/comments/:id/replies?offset=...&limit=20`: `{data,count}`, at most 100 replies. Moderator-hidden roots are inaccessible with their replies. Deleted/withdrawn roots remain placeholders while other active members have visible replies.
...
- Admin: `GET /api/admin/stats`, `GET /api/admin/comments?status=all|normal|moderated|deleted|withdrawn&page_id=...&offset=0&limit=20`; `PUT /api/admin/comments/:id/moderation {hidden:true,category,detail}` or `{hidden:false}`. Deleted comments cannot be restored.
- `GET /api/health`: `{status:"ok",gitSha,workerVersionId,schemaVersion:2,maintenance}` after real table/column probes. An unsupported schema gives HTTP 503; a missing table gives HTTP 500, not healthy status. `workerVersionId` is Cloudflare's version metadata binding; it is null only in local emulation without that binding. Remote deployment verification must require a nonempty ID.
...
Account withdrawal now suspends the member for exactly 30×24 hours. Same-sub authentication issues only a recovery proof; explicit recovery restores the old ID and eligible contributions, never deleted/moderated text or the admin role. Expiry releases identity/nickname and bounded hourly purge preserves other members' replies and shared tags. See `../docs/account-withdrawal-google-design.md` for the authoritative policy.

Schema 2 is required before deploying this code. `0002_withdrawal_requests.sql` is additive; do not rerun 0001 against an existing schema. Stage and production require separate 64-hex-character `WITHDRAWAL_ENCRYPTION_KEY` secrets and `WITHDRAWAL_ENABLED=true` to enable withdrawal. Turning the feature off must not stop expiry cleanup or allow withdrawn users to log in.

Withdrawal endpoints: POST `/api/account/withdrawal/start|confirm|cancel`, GET `/api/account/withdrawal/status`. DELETE `/api/account` aliases confirm, not immediate erasure. Recovery endpoints: GET `/api/account/recovery`, POST `/api/account/recovery/confirm|cancel`. Recovery mutations use the recovery cookie/CSRF, not a normal session. Status is available with a short result cookie after logout.
...
Once withdrawal states exist, never roll back to schema-1 code that ignores member status. Only a version that blocks withdrawn/purging members is compatible.
