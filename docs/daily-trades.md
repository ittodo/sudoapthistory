    ...
    async function refresh({keepPlaying=false}={}){
      ...
      try{
        ...
        if(mode==='day'){
          ...
          const s=M.summary(rows);status=`${day} · ${s.count.toLocaleString()}건 · 신고가 ${s.high} · 하락 ${s.down} · 신저가 ${s.low} · 위치 미확인 ${s.unlocated}건`;
        }else{
...
예약 생성은 `--previous-site`로 직전 사이트를 파일별 캐시로 사용한다. 이전 manifest의 압축 해시와 압축 해제 내용을 모두 확인하여 동일한 월별 자료는 재압축하지 않는다. 손상·누락·내용 변경 시 재생성한다. 논리적인 index 내용이 같으면 updated도 유지하고, 같은 대상 파일은 다시 쓰지 않는다. 전체 가격 판정은 계속 재계산하므로 과거 해제·정정의 후속 기록 영향도 반영한다. 데이터 스캔 자체를 생략하는 월별 증분 계산은 아니다.

배포 패키징도 기존 산출물과 바이트가 같으면 보존하고 변경·손상 파일만 쓴다. 현재 공개 목록에서 빠진 산출물은 제거하며 symlink는 거부한다. 전체 파일 검증은 유지한다. 새 CI 실행기는 빈 작업 공간일 수 있으므로 로컬 패키징 재사용과 원격 자산 중복 업로드 방지는 별개다.
