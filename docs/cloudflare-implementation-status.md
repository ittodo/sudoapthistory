...
- 스키마 2, 전체 세션/쓰기 차단, 닉네임 예약, 공개 댓글/집계 제외, bounded 만료 정리를 구현했다.
- 복구는 같은 Google sub/내부 ID/탈퇴 회차와 마감을 검사하며 관리자 역할·직접 삭제·관리자 숨김은 복원하지 않는다.
- 가상 API/보안 시험 15개와 정적/배포 도구 시험 19개 통과. 취소·회차·동시 정리 경쟁도 확인했다. 타입 검사 및 공개 자산 2,980개 빌드 성공.
- 시험 배포에서 기존 키를 유지하거나 시험 전용 암호화 키를 최초 등록하는 절차를 추가했다. 비밀 값은 출력/파일 저장하지 않는다.
- 실제 시험 배포/Google 철회·복구 사용자 확인 및 운영 초기 migration 이력 정합화는 아직 미완료다.
- 개인정보 초안/복구 안내를 갱신했다. 법적 미확정 초안은 공개 자산에 포함하지 않는다.
- 기존 영구 삭제 자료·Supabase 이관·회원 매핑·역이관·DNS 변경 없음. 상세는 account-withdrawal-google-design.md.

### 2026-09-13 삭제 정책 시험 배포 완료

- 사용자 시험 배포 승인 후 `cloudflare-staging`에 `be5f4f3acb159b09dd7926c79f1d99d5499ae72c` 반영.
- [시험 배포 34734511437](https://github.com/ittodo/sudoapthistory/actions/runs/34734511437) 검사·배포·공개 검증·Google 로그인 시작 검사·artifact 저장 모두 성공.
- 활성 시험 Worker `6eb50514-906f-457b-92d6-8b246d6ec09c`, 스키마 1.
  별도 공개 확인에서도 SHA와 manifest 해시 `c9c9865a5ee1b0159bfed234980c64610a55c87892cacd1bac8122a47fc11abe` 일치.
- 본인 댓글 삭제 시 내용·작성자 연결 제거 및 관리자 숨김 30일 후 정리 코드가 시험 환경에 적용됐다.
  30일 경계/복원/타인 답글 보존은 가상 DB 회귀 검사 통과. 실제 시험 DB의 만료 기록을
  강제로 만들거나 정리 작업을 수동 실행하지 않았다. 로그인 후 실제 삭제 UI 시험은 별도다.
- 공개 자산 2,980개. 개인정보 초안은 저장소 기록에 포함되지만 Workers 공개 자산에서 제외된다.
- main·운영 Worker·DNS·요금제 변경 없음. 이 배포 결과 기록은 배포 후 로컬 문서 변경이다.
