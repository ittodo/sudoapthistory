...
2026-09-13 사용자 최종 승인으로 시험·운영의 매회 수동 배포 승인을 사용하지 않는다.
시험 환경은 `cloudflare-staging` 브랜치만, 운영 환경은 `main` 브랜치만 허용한다.
검사 성공 후 자동 배포하며 첫 운영 전환·DNS·인증/쓰기 활성화·유료 가입의 별도 확인은 유지한다.
...
- Staging workflow는 검사 성공 이후 고정된 시험 계정·DB·URL·브랜치·점검 모드를 검사하고 자동 배포한다. 로그인 활성화 시 승인된 Google client ID를 검사한다. 이 검사를 통과한 **기존 시험 DB에만** `wrangler d1 migrations apply nodostream-staging --remote`로 미적용 스키마를 적용한다. DB 생성·데이터 가져오기·운영 migration은 하지 않는다. Production workflow는 여전히 자동 migration하지 않는다.
...
- 검증된 커밋을 시험 브랜치로 push하면 checks 성공 후 자동 배포한다. 같은 SHA health·manifest·대표 JSON/HTML 해시·없는 JSON 404가 성공해야 한다.
