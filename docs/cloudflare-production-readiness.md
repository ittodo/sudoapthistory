...
| 항목 | 확인 결과 / 남은 일 |
|---|---|
| 시험 배포 | 실행 `34731943494` 성공, 공개 SHA `665f3ebbb6062c9cf65282a54f1c04b58704fd08` |
| 시험 Worker | `7589cf27-d109-4c1a-9a84-ff76472001f3`, 스키마 1, 점검 모드 해제 |
| 운영 Worker | 비공개 준비 실행 `34732319750` 성공. 이후 Google 변수 저장으로 활성 버전 `370c158d` 적용. workers.dev·preview 비활성 유지, 인증·쓰기 차단 |
...
| 운영 DB·비밀 설정 | 스키마 1·사용자 테이블 0건 및 운영 Worker 배포 확인 완료. 운영 전용 Google 클라이언트 ID·암호화 비밀키 저장 완료 |
...
| 정식 로그인 | 운영 전용 callback 등록 완료. 홈페이지·개인정보·승인 도메인 최종 확인·Google 공개 상태 정리 및 실제 운영 로그인 시험 필요 |
| 일정·DNS·과금 | 변경하지 않음 |
